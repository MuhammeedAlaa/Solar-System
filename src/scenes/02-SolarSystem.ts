...
export default class SolarSystemScene extends Scene {
    ...
    private drawSystem(parent: mat4, system: SolarSystemDescription){
        ...
        if(system.children)
        for(let ch of system.children) 
        {
            let child = mat4.clone(parent);
            mat4.translate(child, child, [ch.distanceFromParent * Math.cos(this.time * ch.rotationSpeedAroundParent), 0, -ch.distanceFromParent * Math.sin(this.time * ch.rotationSpeedAroundParent)]);
